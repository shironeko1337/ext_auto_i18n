...
with open('config.yaml', 'rb') as f:
  ...
  configFiles = [file if file.endswith('.yaml') else file +'.yaml' for file in sys.argv[1:]] if len(sys.argv) > 1 \
  else [file for file in [*os.walk('.')][0][2] if file.endswith('yaml') and file != 'config.yaml']

  print('Generating i18n messages.json from',*configFiles,'...')
    ...
    with open(file, 'rb') as f:
      ...
      if data:
        ...
        with open(cacheFile,'w') as f:
            ...
            print('Generation complete, cache dumped.')
